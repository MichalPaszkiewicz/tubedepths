var stationDepths = [ 
    {
        "name": "Northolt ",
        "groundLevel": 43,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 37.80000000000001,
                "southOrWestbound": 37.80000000000001
            }
        ]
    },
    {
        "name": "Greenford ",
        "groundLevel": 22,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 32.19999999999999,
                "southOrWestbound": 32.19999999999999
            }
        ]
    },
    {
        "name": "Perivale ",
        "groundLevel": 20,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 26.599999999999994,
                "southOrWestbound": 26.599999999999994
            }
        ]
    }, 
    {
        "name": "Hanger Lane ",
        "groundLevel": 40,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 31.900000000000006,
                "southOrWestbound": 31.900000000000006
            }
        ]
    },
    {
        "name": "North Acton ",
        "groundLevel": 33,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 24.400000000000006,
                "southOrWestbound": 24.400000000000006
            }
        ]
    },
    {
        "name": "East Acton ",
        "groundLevel": 12,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 17.900000000000006,
                "southOrWestbound": 17.900000000000006
            }
        ]
    },
    {
        "name": "White City ",
        "groundLevel": 7,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 5,
                "southOrWestbound": 5
            }
        ]
    },
    {
        "name": "Shepherd's Bush",
        "groundLevel": 6,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": -10.200000000000003,
                "southOrWestbound": -10.200000000000003
            }
        ]
    },
    {
        "name": "Holland Park ",
        "groundLevel": 14,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": -4.799999999999997,
                "southOrWestbound": -4.799999999999997
            }
        ]
    },
    {
        "name": "Notting Hill Gate ",
        "groundLevel": 28,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": -2.9000000000000057,
                "southOrWestbound": 2.5999999999999943
            },
            {
                "name": "Circle",
                "northOrEastbound": 19.400000000000006,
                "southOrWestbound": 19.400000000000006
            },
            {
                "name": "District",
                "northOrEastbound": 19.400000000000006,
                "southOrWestbound": 19.400000000000006
            }   
        ]
    },
    {
        "name": "Queensway ",
        "groundLevel": 27,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 3.0999999999999943,
                "southOrWestbound": 3.0999999999999943
            }
        ]
    },
    {
        "name": "Lancaster Gate ",
        "groundLevel": 18,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 2.5,
                "southOrWestbound": 2.5
            }
        ]
    },
    {
        "name": "Marble Arch ",
        "groundLevel": 27,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 1.4000000000000057,
                "southOrWestbound": 1.4000000000000057
            }
        ]
    },
    {
        "name": "Bond Street ",
        "groundLevel": 22,
        "stationLines": [
            {
                "name": "Central",
                "northOrEastbound": 1,
                "southOrWestbound": 1
            },
            {
                "name": "Jubilee",
                "northOrEastbound": -9,
                "southOrWestbound": -9
            }
        ]
    },
    {
        "name": "Green Park ",
        "groundLevel": 18,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -13,
                "southOrWestbound": -13
            },
            {
                "name": "Piccadilly",
                "northOrEastbound": -8.5,
                "southOrWestbound": -8.5
            },
            {
                "name": "Victoria",
                "northOrEastbound": -5.5,
                "southOrWestbound": -4.700000000000003
            }
        ]
    },
    {
        "name": "Westminster ",
        "groundLevel": 6,
        "stationLines": [
            {
                "name": "Circle",
                "northOrEastbound": -1,
                "southOrWestbound": -1
            },
            {
                "name": "District",
                "northOrEastbound": -1,
                "southOrWestbound": -1
            },
            
            {
                "name": "Jubilee",
                "northOrEastbound": -16,
                "southOrWestbound": -25.400000000000006
            }
        ]
    },
    {
        "name": "Waterloo ",
        "groundLevel": 4,
        "stationLines": [
            {
                "name": "Bakerloo",
                "northOrEastbound": -14.900000000000006,
                "southOrWestbound": -11.799999999999997
            },
            {
                "name": "Jubilee",
                "northOrEastbound": -26,
                "southOrWestbound": -26
            },
            {
                "name": "Northern",
                "northOrEastbound": -17.200000000000003,
                "southOrWestbound": -17.200000000000003
            },
            {
                "name": "Waterloo & City",
                "northOrEastbound": -1.7999999999999972,
                "southOrWestbound": -1.7999999999999972
            }
        ]
    },
    {
        "name": "Southwark ",
        "groundLevel": 4,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -20.5,
                "southOrWestbound": -20.5
            }
        ]
    },
    {
        "name": "London Bridge ",
        "groundLevel": 4,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -23.200000000000003,
                "southOrWestbound": -23.200000000000003
            },
            {
                "name": "Northern",
                "northOrEastbound": -19.599999999999994,
                "southOrWestbound": -19.599999999999994
            }
        ]
    },
    {
        "name": "Bermondsey ",
        "groundLevel": 2,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -12.5,
                "southOrWestbound": -12.5
            }
        ]
    },
    {
        "name": "Canada Water ",
        "groundLevel": 5,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -13.299999999999997,
                "southOrWestbound": -13.299999999999997
            }
        ]
    },
    {
        "name": "Canary Wharf ",
        "groundLevel": 3,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -15.599999999999994,
                "southOrWestbound": -15.599999999999994
            }
        ]
    },
    {
        "name": "North Greenwich ",
        "groundLevel": 2,
        "stationLines": [
            {
                "name": "Jubilee",
                "northOrEastbound": -13.5,
                "southOrWestbound": -13.5
            }
        ]
    }
]
//.sort((a,b) => b.groundLevel - a.groundLevel);

var stationDetails = [{"name":"Waterloo","localAuthority":"Lambeth","opened":"10 March 1906","zones":[1],"usageMillionsPerYear":100.36},{"name":"Bermondsey","localAuthority":"Southwark","opened":"17 September 1999","zones":[2],"usageMillionsPerYear":11.07},{"name":"Bond Street","localAuthority":"City of Westminster","opened":"24 September 1900","zones":[1],"usageMillionsPerYear":39.53},{"name":"Canada Water","localAuthority":"Southwark","opened":"17 September 1999","zones":[2],"usageMillionsPerYear":14.44},{"name":"Canary Wharf","localAuthority":"Tower Hamlets","opened":"17 September 1999","zones":[2],"usageMillionsPerYear":54.79},{"name":"East Acton","localAuthority":"Hammersmith and Fulham","opened":"03 August 1920","zones":[2],"usageMillionsPerYear":4.12},{"name":"Green Park","localAuthority":"City of Westminster","opened":"15 December 1906","zones":[1],"usageMillionsPerYear":41.24},{"name":"Greenford","localAuthority":"Ealing","opened":"30 June 1947","zones":[4],"usageMillionsPerYear":4.81},{"name":"Hanger Lane","localAuthority":"Ealing","opened":"30 June 1947","zones":[3],"usageMillionsPerYear":4.1},{"name":"Holland Park","localAuthority":"Kensington and Chelsea","opened":"30 July 1900","zones":[2],"usageMillionsPerYear":3.23},{"name":"Lancaster Gate","localAuthority":"City of Westminster","opened":"30 July 1900","zones":[1],"usageMillionsPerYear":7.06},{"name":"London Bridge","localAuthority":"Southwark","opened":"25 February 1900","zones":[1],"usageMillionsPerYear":70.74},{"name":"Marble Arch","localAuthority":"City of Westminster","opened":"30 July 1900","zones":[1],"usageMillionsPerYear":14.11},{"name":"North Acton","localAuthority":"Ealing","opened":"05 November 1923","zones":[2,3],"usageMillionsPerYear":6.53},{"name":"North Greenwich","localAuthority":"Greenwich","opened":"14 May 1999","zones":[2,3],"usageMillionsPerYear":26.45},{"name":"Northolt","localAuthority":"Ealing","opened":"21 November 1948","zones":[5],"usageMillionsPerYear":5.16},{"name":"Notting Hill Gate","localAuthority":"Kensington and Chelsea","opened":"1 October 1868","zones":[1,2],"usageMillionsPerYear":17.18},{"name":"Perivale","localAuthority":"Ealing","opened":"30 June 1947","zones":[4],"usageMillionsPerYear":2.46},{"name":"Queensway","localAuthority":"City of Westminster","opened":"30 July 1900","zones":[1],"usageMillionsPerYear":8.71},{"name":"Shepherd's Bush","localAuthority":"Hammersmith and Fulham","opened":"30 July 1900","zones":[2],"usageMillionsPerYear":22.85},{"name":"Southwark","localAuthority":"Southwark","opened":"24 September 1999","zones":[1],"usageMillionsPerYear":17.86},{"name":"Westminster","localAuthority":"City of Westminster","opened":"24 December 1868","zones":[1],"usageMillionsPerYear":28.38},{"name":"White City","localAuthority":"Hammersmith and Fulham","opened":"23 November 1947","zones":[2],"usageMillionsPerYear":8.95}];

function getStationDetails(name){
    return stationDetails.filter(sd => sd.name.trim() == name.trim())[0];
}

var canvas = document.getElementById("my-canvas");

var seperatorLength = 70;
var height = 800;
var width = (stationDepths.length + 1) * seperatorLength;
var platformSize = 13;
var fontSize = 11;
var font =  fontSize + "px Arial";
var topMargin = 100;
var maxUsage = Math.max(...stationDetails.map(sd => sd.usageMillionsPerYear));
var usageScale = 100;
var zoneDiagramHeight = topMargin - 15;

var maxNum = 0;
var minNum = 0;

stationDepths.forEach(sd => {
    maxNum = Math.max(sd.groundLevel, maxNum);
    sd.stationLines.forEach(sl => {
        maxNum = Math.max(sl.northOrEastbound, sl.southOrWestbound, maxNum);
    });
    minNum = Math.min(sd.groundLevel, minNum);
    sd.stationLines.forEach(sl => {
        minNum = Math.min(sl.northOrEastbound, sl.southOrWestbound, minNum);
    });
});

var normalisationConstant = (height/2 - topMargin) / Math.max(maxNum, Math.abs(minNum));

canvas.height = height;
canvas.width = width;

var context = canvas.getContext("2d");
context.font=font;
context.textAlign="center";

function writeStationNames(ctx, sd){
    var iterator = 1;
    ctx.strokeStyle = "rgb(220,220,220)";    
    ctx.setLineDash([]);    
    sd.forEach(x => {
        var words = (x.name + "(" + getStationDetails(x.name).opened.split(" ")[2] + ")").split(" ");
        ctx.beginPath();
        var j = 1;
        words.forEach(w => {
            ctx.fillText(w, iterator * seperatorLength, j * (fontSize + 2));
            j++;
        });
        ctx.moveTo(iterator * seperatorLength, 3 + j * fontSize);
        ctx.lineTo(iterator * seperatorLength, height - 15);
        ctx.stroke();
        iterator++;
        ctx.closePath();
    });
    ctx.setLineDash([]);
}


function drawSeaLevel(ctx){
    ctx.beginPath();
    ctx.fillStyle = "rgba(180, 200, 255, 0.4)"
    ctx.fillRect(0, height/2, width, height);
    ctx.closePath();
    ctx.beginPath();
    ctx.strokeStyle = "darkblue";
    ctx.setLineDash([8,3,2,3]);
    ctx.moveTo(0, height/2);
    ctx.lineTo(width, height / 2);
    ctx.stroke();
    ctx.closePath();
    ctx.strokeStyle = "black";
    ctx.setLineDash([]);
}

function getMatchingZone(station1, station2){
    var sd1 = getStationDetails(station1.name);
    var sd2 = getStationDetails(station2.name);

    for(var i = 0; i < sd1.zones.length; i++){
        for(var k = 0; k < sd2.zones.length; k++){
            if(sd1.zones[i] == sd2.zones[k]){
                return sd1.zones[i];
            }
        }
    }
    return null;    
}

function getCorrectestZone(station1, station2){
    var sd1 = getStationDetails(station1.name);
    var sd2 = getStationDetails(station2.name);

    var sorted = sd1.zones.sort((a,b) => Math.abs(a - sd2.zones[0]) - Math.abs(b - sd2.zones[0]));

    return sorted[0];

    return Math.max(...(sd1.zones.concat(sd2.zones)));
}

function drawZones(ctx, sd){
    var splitSize = 3;
    var zoneOffset = 10;
    ctx.beginPath();
    ctx.moveTo(0, zoneDiagramHeight);
    var lastZone = -1;
    var previousLeft = 0;
    for(var i = 1; i < sd.length; i++){
        var zone = getMatchingZone(sd[i-1], sd[i])
        var c = 1;
        if(zone == null){
            c = 0.5;
            zone = getCorrectestZone(sd[i-1], sd[i])
        }
        var farRight = (i == sd.length - 1) ? width : (i + c) * seperatorLength;
        for(var j = 0; j < zone; j++){
            ctx.moveTo(previousLeft, zoneDiagramHeight - j * splitSize);
            ctx.lineTo(farRight, zoneDiagramHeight - j * splitSize);
            
        }
        if(zone != lastZone){
            ctx.fillStyle="black";
            ctx.textAlign="right";
            ctx.fillText(lastZone, previousLeft - zoneOffset, zoneDiagramHeight - (lastZone + 1) * splitSize);
            ctx.textAlign="left"
            ctx.fillText(zone, previousLeft + zoneOffset, zoneDiagramHeight - (zone + 1) * splitSize);            
            ctx.textAlign="center";
            lastZone = zone;
        }
        previousLeft = farRight;
    }
    ctx.stroke();
    ctx.closePath();
}

function drawGroundLevel(ctx, sd){
    drawZones(ctx, sd);

    var iterator = 1;
    ctx.beginPath();
    ctx.strokeStyle = "brown";
    ctx.setLineDash([5,3]);
    ctx.moveTo(0, height/2 - normalisationConstant * sd[0].groundLevel);
    sd.forEach(x => {
        ctx.lineTo(iterator * seperatorLength, height/2 - normalisationConstant * x.groundLevel);
        iterator++;
    });
    ctx.lineTo(width, height / 2 - normalisationConstant * sd[sd.length - 1].groundLevel);
    
    ctx.stroke();
    ctx.closePath();
    ctx.strokeStyle = "black";
    ctx.setLineDash([]);
}

function getColour(line){
    var lineColours = [{line:"Bakerloo",colour:"brown"},
    {line:"Central",colour:"red"},
    {line:"District",colour:"green"},
    {line:"Circle", colour:"#f5ef02"},
    {line:"Hammersmith & City",colour:"pink"},
    {line:"Jubilee",colour:"grey"},
    {line:"Metropolitan",colour:"maroon"},
    {line:"Northern",colour:"black"},
    {line:"Piccadilly",colour:"blue"},
    {line:"Victoria",colour:"deepskyblue"},
    {line:"Waterloo & City",colour:"turquoise"}];

    return lineColours.filter(lc => lc.line == line)[0].colour;
}

function matchLines(station1, station2){
    var matchedLines = [];
    for(var i = 0; i < station1.stationLines.length; i++){
        for(var j = 0; j < station2.stationLines.length; j++){
            if(station1.stationLines[i].name == station2.stationLines[j].name){
                if(!matchedLines.some(ml => ml == station1.stationLines[i].name)){
                    matchedLines.push(station1.stationLines[i].name);
                }
            }
        }
    }
    return matchedLines;
}

function getPlatformsWithLines(station, lines){
    var platforms = [];
    for(var i = 0; i < station.stationLines.length; i++){
        if(lines.some(l => station.stationLines[i].name == l)){
            platforms.push(station.stationLines[i]);
        }
    }
    return platforms;
}

function drawJourney(ctx, sd){
    ctx.setLineDash([]);
    ctx.lineWidth = 5;

    for(var i = 1; i < sd.length; i++){
        var ml = matchLines(sd[i - 1], sd[i])
        
        var platforms1 = getPlatformsWithLines(sd[i - 1], ml);
        var platforms2 = getPlatformsWithLines(sd[i], ml);

        for(var j = 0; j < platforms1.length; j++){
            for(var k = 0; k < platforms2.length; k++){
                ctx.beginPath();
                ctx.moveTo(i * seperatorLength, height/2 - normalisationConstant * (platforms1[j].northOrEastbound));
                ctx.strokeStyle = getColour(platforms1[j].name);
                ctx.lineTo((i + 1) * seperatorLength, height/2 - normalisationConstant * (platforms2[j].northOrEastbound));
                ctx.stroke();
                ctx.closePath();      
                ctx.beginPath();
                ctx.moveTo(i * seperatorLength, height/2 - normalisationConstant * (platforms1[j].southOrWestbound));
                ctx.strokeStyle = getColour(platforms1[j].name);
                ctx.lineTo((i + 1) * seperatorLength, height/2 - normalisationConstant * (platforms2[j].southOrWestbound));
                ctx.stroke();
                ctx.closePath();    
            }
        }
    }
    ctx.lineWidth = 1;
}

function drawPlatform(ctx, x, y, colour, hasl, leftText, gl){
    ctx.beginPath();
    ctx.strokeStyle="white";
    ctx.fillStyle = colour;      
    ctx.arc(x, y, platformSize, 0,2*Math.PI);   
    ctx.fill();
    if(leftText){
        ctx.textAlign = "right";
        ctx.fillText(~~hasl, x - platformSize*1.4, y + platformSize / 2);
        ctx.stroke();
    }
    else{
        ctx.textAlign = "left";
        ctx.fillText(~~hasl, x + platformSize*1.4, y + platformSize / 2);
        ctx.stroke();
    }
    ctx.fillStyle="white";
    ctx.textAlign = "center";    
    ctx.fillText(~~(hasl - gl), x, y + platformSize / 2 - 2);
    ctx.closePath();         
}

function drawDepths(ctx, sd){
    var iterator = 1;
    sd.forEach(x => {

        var i = (x.stationLines.length > 1) ? -5 : 0;

        var thisStationDetail = getStationDetails(x.name);

        ctx.beginPath();

        ctx.fillStyle="blue";
        ctx.fillRect(iterator * seperatorLength - 5, height - thisStationDetail.usageMillionsPerYear * usageScale / maxUsage, 10, thisStationDetail.usageMillionsPerYear * usageScale / maxUsage);
        ctx.fillText(thisStationDetail.usageMillionsPerYear + "m", iterator * seperatorLength, height - thisStationDetail.usageMillionsPerYear * usageScale / maxUsage - 10);
        ctx.closePath();

        x.stationLines.forEach(sl => {
            drawPlatform(ctx, iterator * seperatorLength + i, height/2 - normalisationConstant * (sl.northOrEastbound), getColour(sl.name), sl.northOrEastbound, i < 0, x.groundLevel);
            if(Math.abs(sl.southOrWestbound - sl.northOrEastbound) > 3){
                drawPlatform(ctx, iterator * seperatorLength + i, height/2 - normalisationConstant * (sl.southOrWestbound), getColour(sl.name), sl.southOrWestbound, i < 0, x.groundLevel);
            }
            i+=7;
        });
        iterator++;
    });
    ctx.fillStyle = "black";
    ctx.setLineDash([]);
}


var year = 1900;
var yearIncrement = 1;
var testYear = new Date(year, 0, 0);
var yearlyStationDepths;
var now = new Date();

writeStationNames(context, stationDepths);
drawSeaLevel(context);
drawGroundLevel(context, stationDepths);
drawJourney(context, stationDepths);  
drawDepths(context, stationDepths);  

function animate(){
    yearlyStationDepths = stationDepths.filter(sd => new Date(getStationDetails(sd.name).opened) < testYear);

    context.clearRect(0,0,width,height);
    writeStationNames(context, yearlyStationDepths);
    drawSeaLevel(context);
    drawGroundLevel(context, yearlyStationDepths);
    drawJourney(context, yearlyStationDepths);  
    drawDepths(context, yearlyStationDepths);  

    if(testYear > now){
        
         
        return;
    }
    else{
        context.fillStyle = "black";
        context.fillText(year, 10, 10);
    }
    year += yearIncrement;
    testYear = new Date(year, 0, 0)

    setTimeout(() => {
        draw();
    }, 200);
}

//animate();